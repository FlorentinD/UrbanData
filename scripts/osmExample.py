...
for type, tags in tagsPerType.items():
    ...
    popularTagCombinations= sorted(tagCombinations.items(), key=lambda kv: kv[1], reverse=True)
    print("Most used tags for {}".format(type))
    [print("\t {}: {}".format(comb[0], comb[1])) for comb in list(popularTagCombinations)[:10]]